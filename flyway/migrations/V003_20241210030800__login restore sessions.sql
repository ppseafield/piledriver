...
RETURNS TABLE(user_id uuid, username varchar(64), email text, session_id uuid, expires_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE founduser pd_users%ROWTYPE;
BEGIN
    SELECT u.*
    INTO founduser
   FROM pd_users u
   INNER JOIN pd_passwords p
   ON u.id = p.user_id
  ...
  IF NOT FOUND THEN
      ...
      return QUERY
      WITH newsession AS (
          INSERT INTO pd_sessions (user_id) VALUES (founduser.id) RETURNING *
      )
...
$$;
GRANT ALL ON function current_user_id TO piledriver_user;
...
-- TODO: make this work???
CREATE OR REPLACE FUNCTION check_session_expiration() RETURNS void AS $$
DECLARE
  session_id UUID := current_setting('request.jwt.claims', true)::json->>'session_id';
...
END
$$ language plpgsql;
GRANT ALL ON function check_session_expiration TO piledriver_user;
